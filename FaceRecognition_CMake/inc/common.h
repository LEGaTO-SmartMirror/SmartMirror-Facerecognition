...
#define CUDA_CHECK(status)                                             \
	...
	{                                                                  \
		...
		if (ret != 0)                                                  \
		{                                                              \
			TrtLog::gLogError << "Cuda failure: " << ret << std::endl; \
			abort();                                                   \
		}                                                              \
	} while (0)
...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                                                                            \
	...
	{                                                                                                                      \
		if (!(status))                                                                                                     \
		{                                                                                                                  \
			TrtLog::gLogError << errMsg << " Error in " << __FILE__ << ", function " << FN_NAME << "(), line " << __LINE__ \
							  << std::endl;                                                                                \
			return val;                                                                                                    \
		}                                                                                                                  \
	} while (0)
...
#define ASSERT(condition)                                                          \
	...
	{                                                                              \
		if (!(condition))                                                          \
		{                                                                          \
			TrtLog::gLogError << "Assertion failure: " << #condition << std::endl; \
			abort();                                                               \
		}                                                                          \
	} while (0)
...
makeObjGuard(T_* t)
{
	CUDA_CHECK(!(std::is_base_of<T, T_>::value || std::is_same<T, T_>::value));
...
class HostMemory : public nvinfer1::IHostMemory
{
public:
	HostMemory() = delete;
...
inline void setDummyInt8Scales(const nvinfer1::IBuilderConfig* c, nvinfer1::INetworkDefinition* n)
{
...
inline void loadLibrary(const std::string& path)
{
	...
	{
#ifdef _MSC_VER
		TrtLog::gLogError << "Could not load plugin library: " << path << std::endl;
#else
		TrtLog::gLogError << "Could not load plugin library: " << path << ", due to: " << dlerror() << std::endl;
#endif
	}
}
